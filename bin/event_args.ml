...
let parse_start ~start_date ~start_time ~timezone =
  ...
  | Some start_date -> (
      ...
      | None ->
          ...
          in
          let* ptime =
            Date.parse_date start_date `From () ~tz:Timedesc.Time_zone.utc
          in
      ...
      | Some start_time -> (
          ...
          | None ->
              let* datetime =
                Date.parse_date_time ~date:start_date ~time:start_time `From ()
              in
          ...
          | Some tzid ->
              ...
              in
              let* datetime =
                Date.parse_date_time ~date:start_date ~time:start_time `From ~tz
                  ()
              in
...
let parse_end ~end_date ~end_time ~timezone ~end_timezone =
  ...
  | Some end_date -> (
      ...
      | None ->
          ...
          in
          let* ptime =
            Date.parse_date end_date `From () ~tz:Timedesc.Time_zone.utc
          in
          let date = Ptime.to_date ptime in
      ...
      | Some end_time -> (
          ...
          | None, None ->
              let* datetime =
                Date.parse_date_time ~date:end_date ~time:end_time `From ()
              in
              Ok
                (Some
          ...
          | _, Some tzid | Some tzid, _ ->
              ...
              in
              let* datetime =
                Date.parse_date_time ~date:end_date ~time:end_time `From ~tz ()
              in
